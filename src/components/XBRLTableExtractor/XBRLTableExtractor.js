...
const XBRLTableExtractor = () => {
  ...
  const [xbrlData, setXbrlData] = useState([]);
  const [isProcessing, setIsProcessing] = useState(false);
  const [activeTableIndex, setActiveTableIndex] = useState(0);
  const [showXbrlData, setShowXbrlData] = useState(false);
  ...
  const extractTablesAndXbrl = useCallback(() => {
    ...
    try {
      ...
      const xbrlTags = Array.from(xbrlElements).map(el => {
        return {
      ...
      const extractedData = Array.from(tables).map((table, index) => {
        ...
        const rowsData = rows.map(row => {
          ...
          return cells.map(cell => {
            ...
            if (xbrlElements.length > 0) {
              const xbrlInfo = Array.from(xbrlElements).map(el => {
                return {
                  name: el.tagName.toLowerCase(),
                  contextRef: el.getAttribute('contextref'),
                  value: el.textContent.trim(),
        ...
        return {
          ...
          hasXbrl: rowsData.some(row => row.some(cell => typeof cell === 'object' && cell.xbrl))
        };
      });
  ...
  const getDisplayData = useCallback(() => {
    ...
    return tableData.map(row => 
      row.map(cell => {
        if (typeof cell === 'object' && cell.xbrl) {
          return showXbrlData 
        ...
        <div className="extracted-tables mt-6">
          ...
          {xbrlData.length > 0 && (
            <div className="xbrl-data-summary mt-6">
              <h3 className="text-lg font-semibold mb-2">XBRLデータサマリー</h3>
              <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded-md">
                <p>検出されたXBRLタグ: {xbrlData.length}個</p>
                <p>主なコンテキスト: {Array.from(new Set(xbrlData.map(x => x.contextRef))).slice(0, 5).join(', ')}</p>
                <p>主な単位: {Array.from(new Set(xbrlData.map(x => x.unitRef).filter(Boolean))).slice(0, 5).join(', ')}</p>
              </div>
            </div>
          )}
        </div>
