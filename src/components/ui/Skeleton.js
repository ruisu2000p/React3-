 ...
 * 
 * @param {Object} props
 * @param {string} props.type - スケルトンの種類 ('text', 'circle', 'rectangle', 'card')
 * @param {string|number} props.width - スケルトンの幅
 * @param {string|number} props.height - スケルトンの高さ
 * @param {number} props.count - テキストスケルトンの数（段落）
 * @param {boolean} props.animated - アニメーション有効/無効
 */
const Skeleton = ({
  type = 'text',
  width,
